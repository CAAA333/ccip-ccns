...
describe("CCIP Cross Chain Name Service - Register Alice, Lookup Alice and Return her EOA address from both chains", function () {
  ...
  before(async function () {
    ...
    const CrossChainNameServiceLookup = await hre.ethers.getContractFactory(
    ...
    const CrossChainNameServiceRegister = await hre.ethers.getContractFactory(
      "CrossChainNameServiceRegister"
    );
    sourceCrossChainNameServiceRegister =
    ...
    console.log(
      ...
      sourceCrossChainNameServiceRegister.address
    );

    // Deploy contracts on the destination chain
    // Create instance of CrossChainNameServiceLookup
    const DestinationCrossChainNameServiceLookup =
      await hre.ethers.getContractFactory("CrossChainNameServiceLookup");
    destinationCrossChainNameServiceLookup =
      await DestinationCrossChainNameServiceLookup.deploy();
    console.log(
      "CrossChainNameServiceLookup address: ",
    ...
    const CrossChainNameServiceReceiver = await hre.ethers.getContractFactory(
    ...
    let txResponse =
      await sourceCrossChainNameServiceLookup.setCrossChainNameServiceAddress(
        sourceCrossChainNameServiceRegister.address
      );
    console.log(
      "sourceSetCrossChainNameServiceAddress transaction: ",
      txResponse.hash
    );
    txResponse =
      await destinationCrossChainNameServiceLookup.setCrossChainNameServiceAddress(
        crossChainNameServiceReceiver.address
      );
    console.log(
      "destinationSetCrossChainNameServiceAddress transaction: ",
      txResponse.hash
    );
    // Enable chain on CrossChainNameServiceRegister
    txResponse = await sourceCrossChainNameServiceRegister.enableChain(
      config.chainSelector_,
      crossChainNameServiceReceiver.address,
      500_000n
    );
  ...
  it("Should register alice.ccns on the source chain and check that the registering address is the same on both chains", async function () {
    // Connect Alice
    const aliceConnected = await sourceCrossChainNameServiceRegister.connect(
      alice
    );
    console.log("");
    ...
    const sourceLookup = await sourceCrossChainNameServiceLookup.lookup(
      "alice.ccns"
    );
    console.log("Returned source Lookup address: ", sourceLookup);
    const destinationLookup =
      await destinationCrossChainNameServiceLookup.lookup("alice.ccns");
